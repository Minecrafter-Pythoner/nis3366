<template>
    <div class="mb-6 flex items-center justify-between">
        ...
        <p class="text-gray-600">{{ post.created_at_formatted }} 前 </p>
    </div>
    <p>{{ post.body }}</p>
    <div class="my-6 flex justify-between">
        <div class="flex space-x-6">
            <div class="flex items-center space-x-2" @click="likePost(post.id)">
                ...
                <span class="text-gray-500 text-xs">{{ post.likes_count }} 个赞</span>
            </div>
            <div class="flex items-center space-x-2">
                ...
                <span class="text-gray-500 text-xs">0 条评论</span>
            </div>
        </div>
        <div>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
